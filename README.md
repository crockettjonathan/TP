...
A lightweight web application built with Flask and SQLite3 (This database choice was used for simplicity, but any alterntive could be easily retrofitted in) to filter, preview, and export trust review data. This tool allows users to dynamically switch between "Business" and "User" contexts, preview results in the browser, and download a custom-named CSV file.
...
└── 100_prod/
	...
	├── trust.db            # SQLite Database
	└── templates/
		└── index.html      # Frontend Logic

4. Database Preparation
Navigate to the setup folder (\repo\075_setup) in your terminal and run the following to import the data, cleanse it and prepare the database for the application:
